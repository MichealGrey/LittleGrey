...
class EmotionEngine:
    def __init__(self, llm: LLMClient | None = None, logger: AgentLogger | None = None, config: EmotionConfig | None = None):
        ...
        for dim in self._config.dimensions:
            self._baseline[dim] = getattr(self._config, f'baseline_{dim}', 0.0)
        self._mood_journal: list[MoodEntry] = []
        self._baseline_adaptation_rate: float = 0.001
        self._relationship: Any = None
        self._levels: dict[str, float] = {dim: 0.0 for dim in self._config.dimensions}
        self._last_update: float = time.monotonic()
        self._last_result: EmotionResult | None = None
        self._emotion_cache = EmotionCache(max_size=128, threshold=0.85)
    ...
    def _init_decay_strategies(self) -> Dict[str, DecayStrategy]:
        strategies = {}
        for dim in self._config.dimensions:
            strategy_config = self._config.decay_strategies.get(dim)
            if strategy_config:
                if strategy_config.strategy_type == 'exponential':
                    strategies[dim] = ExponentialDecay(rate=strategy_config.rate)
                elif strategy_config.strategy_type == 'linear':
                    strategies[dim] = LinearDecay(duration_hours=strategy_config.duration_hours)
                elif strategy_config.strategy_type == 'step':
                    strategies[dim] = StepDecay(interval_hours=strategy_config.interval_hours, decay_factor=strategy_config.decay_factor)
            else:
                default = self._config.decay_strategies.get('default')
                if default and default.strategy_type == 'exponential':
                    strategies[dim] = ExponentialDecay(rate=default.rate)
                else:
                    strategies[dim] = ExponentialDecay(rate=0.02)
        return strategies

    def understand(self, text: str) -> EmotionResult:
        self._last_interaction_time = time.monotonic()

        cached = self._emotion_cache.get(text)
        if cached is not None:
            self._last_result = cached
            return cached

        if self._loaded_emotion_levels is not None:
            self._decay()
            result = self._llm_understand(text)
            self._merge_with_loaded_emotion(result)
            self._loaded_emotion_levels = None
        else:
            self._decay()
            result = self._llm_understand(text)
    ...
    def render_bar(self) -> str:
        ...
        bar_width = 10
        lines = []
        for emo in self._config.dimensions:
    ...
    def _decay(self) -> None:
        ...
        for emo in self._levels:
            ...
            strategy = self._decay_strategies.get(emo)
            if strategy:
                self._levels[emo] = strategy.calculate(self._levels[emo], baseline, elapsed_hours)
            else:
                decay = math.exp(-0.02 * elapsed_hours)
                self._levels[emo] = baseline + (self._levels[emo] - baseline) * decay
            
            if self._levels[emo] < self._config.min_threshold:
    ...
    def recall_trigger(self, memory_event: dict[str, Any]) -> None:
        """当回忆起特定记忆事件时，重新触发对应情绪。
        
        Args:
            memory_event: 记忆事件，包含 type 和 emotion_impact 字段
        """
        event_type = memory_event.get('type', '')
        emotion_impact = memory_event.get('emotion_impact', {})
        
        trigger_map = {
            'conflict': {'angry': 0.4, 'sad': 0.2, 'anxious': 0.15},
            'threat': {'angry': 0.5, 'anxious': 0.3},
            'insult': {'angry': 0.6, 'sad': 0.3},
            'abandon': {'sad': 0.5, 'anxious': 0.2},
            'caring': {'happy': 0.3, 'excited': 0.1},
            'apology': {'happy': 0.2, 'sad': 0.1},
        }
        
        trigger_emotions = trigger_map.get(event_type, {})
        
        for emo, min_intensity in trigger_emotions.items():
            if emo in self._levels:
                self._levels[emo] = max(self._levels[emo], min_intensity)
        
        for emo, impact in emotion_impact.items():
            if emo in self._levels:
                self._levels[emo] = max(self._levels[emo], min(impact, 0.5))
        
        self._last_update = time.monotonic()
        if any(self._levels.get(e, 0) > 0.1 for e in trigger_emotions):
            self._record_mood(
                emotion=event_type,
                intensity=max(trigger_emotions.values()) if trigger_emotions else 0.3,
                trigger=f"memory_recall: {event_type}"
            )
    ...
    def _save_emotion_state(self, long_term_memory: Any = None) -> None:
        """Save emotion state to JSON file."""
        try:
            recent_journals = [
                {
            ...
            self._storage.save(state_data)
        except Exception as e:
            pass

    def _load_emotion_state(self, long_term_memory: Any = None) -> None:
        """Load emotion state from JSON file."""
        try:
            ...
            if saved_levels:
                self._loaded_emotion_levels = {}
                for emo, val in saved_levels.items():
                    if emo in self._levels:
                        val = max(0.0, min(1.0, float(val)))
                        self._levels[emo] = val
                        self._loaded_emotion_levels[emo] = val
            ...
            # Apply decay based on time since save
